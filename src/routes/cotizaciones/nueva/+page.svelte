<script>
	...
	let conceptos = $state([
		{
			descripcion: form?.valores?.descripciones?.[0] ?? '',
			cantidad: Number(form?.valores?.cantidades?.[0] ?? 1),
			precioUnitario: Number(form?.valores?.precios?.[0] ?? 0)
		}
	]);
...
<main class="mx-auto max-w-5xl p-6">
	<h1 class="mb-6 text-2xl font-semibold text-gray-900">Nueva cotización</h1>
			...
			<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
				...
				<div class="flex flex-col gap-1">
					...
					<input
						...
						name="vencimiento"
						type="date"
						value={form?.valores?.vencimiento ?? ''}
					...
					<div class="space-y-3 rounded-xl border border-gray-200 p-4">
						{#each conceptos as row, index (index)}
							<div class="grid grid-cols-1 items-start gap-3 md:grid-cols-12">
								...
								<div class="md:col-span-2">
									<label class="mb-1 block text-xs font-medium text-gray-600">Precio unitario</label
									>
									<input
										name="precioUnitario"
										type="number"
